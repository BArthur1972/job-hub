...
import {
  ...
  useGetEmploymentTypeCountsMutation,
...
function RecruiterAnalytics() {
  ...
  const applicantStatusData = Object.entries(applicantStatusCounts)
  ...
  const employentTypeData = Object.entries(employmentTypeCounts)
    .filter(([_, value]) => value > 0)
    .map(([name, value]) => ({
      name,
      value,
  ...
  const checkForData = () => {
    return (
      !applicantStatusData.every(({ name, value }) => name[value] === 0) &&
      !employentTypeData.every(({ name, value }) => name[value] === 0)
    );
  };
        ...
        <Row>
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            >
              ...
              >
                (Total Nō:{" "}
                {applicantStatusData.reduce((acc, curr) => acc + curr.value, 0)}
                )
              </span>
            </div>
