...
class JobSeeker {
                    ...
                    .then((hashedPassword) => {
                        ...
                        dbConnection.query(query, jobSeeker, (err, result) => {
                            if (err) {
                                ...
                                this.getJobSeekerById(jobSeekerId).then((jobSeeker) => {
                                    resolve(jobSeeker[0]);
                                }).catch((err) => {
                                    reject(err);
                                });
    ...
    static loginJobSeeker(email, password) {
        return new Promise((resolve, reject) => {
            ...
            this.getUserByCredentials(email, password).then((jobSeeker) => {
                resolve(jobSeeker);
            }).catch((err) => {
                reject(err);
            });
    ...
    static generateAuthTokenAndAddToJobSeeker(jobSeeker) {
        return new Promise((resolve, reject) => {
            const token = jwt.sign({ id: jobSeeker.seekerID }, process.env.JWT_SECRET);
